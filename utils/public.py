...
import logging
import Global
from Global import set_config, parent_dir
import os
import shutil
import requests
...
class MyLabel(tk.Label):
    def __init__(self, master, text="", tooltip=None, **kwargs):
        ...
        if tooltip is not None:  # 按钮是否有悬浮提示
            ...
            self.x = self.y = 0
            self.alpha = 0.0
            self.bind("<Enter>", lambda event: self._showtip(event))
            self.bind("<Leave>", lambda event: self.hidetip(event))

    def _fade_in(self):
        if self.alpha < 1.0:
            self.alpha += 0.05  # 增加透明度的值
            self.tipWindow.attributes("-alpha", self.alpha)
            self.id = self.after(50, self._fade_in)

    def _showtip(self, event):
        self.after(100, self.showtip())
...
class BreathingLabel(tk.Label):
    def __init__(self, master=None, image_name='breathing.png', duration=1000, *args, **kwargs):
        ...
        self.image_path = parent_dir + "\\files\\res\\" + image_name
...
class ImageLoader:
    ...
    def load_image(self, name, path):
        image = Image.open(parent_dir + "\\files\\res\\" + path)
...
def clear_folder(folder_path, size):
    if os.path.exists(folder_path) and os.path.isdir(folder_path):
        if get_folder_size(folder_path) >= size * 1024 * 1024:
            shutil.rmtree(folder_path)
            os.mkdir(folder_path)


def check_website_latency(url):
    try:
...
def delay_test_public(urls, url_names):
    results = {}
    with concurrent.futures.ThreadPoolExecutor() as executor:
        futures = {executor.submit(check_website_latency, url): url for url in urls}
