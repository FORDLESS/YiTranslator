...
class BaseTsa:
    ...
    def return_engine(self):
        # 先判断翻译引擎是否存在，是否变更
        if self.trans_engine is not None and (
                Global.api_co == self.now_use_trans or Global.public_trans == self.now_use_trans):
    ...
    def get_target(self, text, fromLang):
        ...
        if _engine is not None:
            try:
                ...
                logging.error(f"翻译时出现错误: {error}")
                return "翻译时出现错误,请前往日志查看"
        else:
            return "未选择翻译接口"
